<x-marketing-layout :title="'Industries — Profit Maximization Consultancy'">
    <section class="border-b border-pmc-silver/80 dark:border-white/10 bg-gradient-to-b from-pmc-navy-light via-pmc-blue-tint/20 to-white dark:from-black/10 dark:via-black/10 dark:to-black/10">
        <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <h1 class="pmc-fade-in-up text-3xl font-semibold text-pmc-charcoal dark:text-white">Industries We Serve</h1>
        ...
        <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ([
                    ...
                    <div class="pmc-card-reveal pmc-hover-lift group relative overflow-hidden rounded-3xl border border-gray-200 dark:border-white/10 bg-gradient-to-b from-gray-50 dark:from-white/10 to-transparent p-1 transition-all hover:border-pmc-blue/30" style="animation-delay: {{ ($index + 1) * 150 }}ms">
                        <div class="relative h-full rounded-[22px] bg-white dark:bg-white/5 p-7">
                            <div class="text-lg font-semibold text-gray-900 dark:text-white pmc-gradient-text">{{ $name }}</div>
                            <div class="mt-3 text-sm text-gray-600 dark:text-slate-300">
                                <div class="text-xs font-semibold text-gray-700 dark:text-slate-200">Typical challenges</div>
                            ...
                            <div class="mt-5 rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-black/10 p-5 text-sm text-gray-700 dark:text-slate-200 pmc-hover-glow">
                                <div class="text-xs font-semibold text-gray-900 dark:text-white">How we solve</div>
                                <div class="mt-1">{{ $solution }}</div>
                                <div class="mt-3 text-xs text-gray-500 dark:text-slate-400">Example metric: <span class="pmc-gradient-text font-semibold">2–5% leakage</span> surfaced early; <span class="pmc-gradient-text font-semibold">5–10% EBITDA</span> uplift potential.</div>
                            </div>
                            <div class="mt-6">
                                <a href="{{ route('contact') }}" class="pmc-hover-scale inline-flex items-center text-sm font-semibold text-pmc-blue transition-all hover:text-gray-900 dark:hover:text-white">
                        ...
                        <div class="pointer-events-none absolute inset-0 opacity-0 transition-all duration-300 group-hover:opacity-100">
                            <div class="absolute inset-0 bg-gradient-to-br from-pmc-blue/15 via-transparent to-blue-600/10"></div>
                        </div>
                    </div>
                @endforeach
            </div>
