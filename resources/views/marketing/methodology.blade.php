<x-marketing-layout :title="'Methodology — Profit Maximization Consultancy'">
    <section class="border-b border-white/10 bg-black/10">
        <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <h1 class="pmc-fade-in-up text-3xl font-semibold text-white">Methodology</h1>
    ...
    <section>
        <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <div x-data="pmcTimeline()" x-init="init()" class="relative">
                <div class="pointer-events-none absolute left-4 top-0 h-full w-px bg-white/10 md:left-1/2"></div>
                    ...
                    <div class="pmc-card-reveal relative grid gap-4 py-8 md:grid-cols-2 md:gap-10" style="animation-delay: {{ ($i + 1) * 200 }}ms">
                        <div class="{{ $i % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:col-start-2 md:pl-12' }}">
                            <div class="inline-flex items-center gap-3">
                                <div class="pmc-hover-scale grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-xs font-semibold text-white transition-all hover:border-pmc-teal/30">
                                    {{ $i + 1 }}
                                </div>
                                <div class="text-xl font-semibold text-white">{{ $title }}</div>
                            </div>
                            <div class="mt-3 text-slate-200">{{ $desc }}</div>
                        </div>
                        <div class="{{ $i % 2 === 0 ? 'md:col-start-2' : 'md:col-start-1 md:row-start-1 md:pr-12 md:text-right' }}">
                            <div class="pmc-hover-lift rounded-3xl border border-white/10 bg-gradient-to-b from-white/10 to-transparent p-1 transition-all hover:border-pmc-emerald/30">
                                <div class="rounded-[22px] bg-white/5 p-6 text-sm text-slate-200">
                                    <div class="text-xs font-semibold text-white">Deliverables</div>
                                    <ul class="mt-3 space-y-2">
                                        <li class="flex gap-2"><span class="mt-1.5 h-1.5 w-1.5 rounded-full bg-pmc-teal pmc-pulse"></span> Executive summary (decision-ready)</li>
                                        <li class="flex gap-2"><span class="mt-1.5 h-1.5 w-1.5 rounded-full bg-pmc-emerald pmc-pulse"></span> Board-level KPI & performance narrative</li>
                        ...
                        <div class="absolute left-4 top-10 md:left-1/2 md:-translate-x-1/2">
                            <div class="pmc-pulse h-4 w-4 rounded-full border border-white/20 bg-gradient-to-br from-pmc-teal to-pmc-emerald shadow pmc-glow"></div>
                        </div>
                    </div>
                @endforeach
            </div>
